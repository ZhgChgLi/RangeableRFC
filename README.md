...
- **[`USECASES.md`](USECASES.md)** — eight worked Ruby examples (calendar free-time finder, genomic annotation diff, game buff/debuff dispel, tax effective-date composition, CIDR / IP allocation, syntax highlighter, ZMediumToMarkdown render, AVPlayer non-contiguous byte cache) showing where `Rangeable` is the natural fit.
